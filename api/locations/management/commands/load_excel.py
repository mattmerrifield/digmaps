...
def period(short, desc="", start=0, end=0):
    t, _ = Period.objects.get_or_create(
        shortname=short, defaults={"description": desc, "start": start, "end": end}
    )
    return t
...
class SiteRecord(object):
        ...
        for field_name in fields:
            ...
            if value and str(value) not in ["nan", "Unknown"]:
                f, _ = Feature.objects.get_or_create(
                    shortname=value, defaults={"name": value}
                )
                yield f
    ...
    def references(self):
        ...
        for title in self.row["Reference"].split(";"):
            r, _ = bibliography.models.Publication.objects.get_or_create(title=title)
            references.append(r)
        return references
    ...
    def site(self):
        ...
        with transaction.atomic():
            ...
            for period in self.periods():
                SitePeriod.objects.get_or_create(site=site, period=period)
            for feature in self.features():
                SiteFeature.objects.get_or_create(site=site, feature=feature)
